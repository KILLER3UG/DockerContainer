...
function getBrainDiagnostics() {
    ...
    checks.push({
        ...
        detail: targetUrl.includes('minimax') && looksLikeProviderAlias(upstreamModel)
            ? `Profile exposes alias ${upstreamModel}; extraction uses ${dedicatedMemoryModel || 'MiniMax-M2.7'}.`
    ...
    return {
        ...
        files: {
            core: coreInfo,
            semantic: getFileInfo(path.join(__dirname, '..', 'august_semantic_memory.json')),
            vector: getFileInfo(path.join(__dirname, '..', 'august_infinite_memory.json'))
        },
        counts: {
